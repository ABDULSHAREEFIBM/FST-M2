-- Load Data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/Shareefmk/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/Shareefmk/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/Shareefmk/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/Shareefmk/outputs;

-- Store Result in HDFS
STORE nameOrdered INTO 'hdfs:///user/Shareefmk/outputs' USING PigStorage('\t');